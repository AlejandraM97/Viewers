...
export const isSeriesMR = () => {
  ...
  if (currentSeriesMetadata.Modality.toLowerCase() == 'mr') {
    return true;
  } else {
    return false;
  }
...
export const makeTransaction = (route, operation, data) => {
  ...
  const requestPayload = {
    ...
    enterprise: enterpriseParam == undefined ? 3 : enterpriseParam,
...
export const getSeriesId = data => {
  ...
  const promisePetition = new Promise((resolve, reject) => {
    ...
    xhttp.open(
      'POST',
      'http://localhost/orthanc/tools/find',
      true
    );
    // xhttp.setRequestHeader(
    //   'Authorization',
